...
- **Standard dev certificate works.** All apps run on `https://localhost` with distinct ports — no wildcard certificate needed. Run `dotnet dev-certs https --trust` once and all five apps will be trusted by the browser.
