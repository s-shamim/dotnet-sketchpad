...
function LogoutApp() {
  ...
  React.useEffect(() => {
    ...
    const url      = `https://identity.localhost:5203/api/ids/logout${logoutId ? `?logoutId=${encodeURIComponent(logoutId)}` : ''}`;

    fetch(url, { credentials: 'include' })
      .then(r => r.json())
      .then(d => {
        setStatus('signed out. redirecting...');
        setTimeout(() => { window.location.href = d.redirectUrl || 'https://shell.localhost:5201'; }, 800);
      })
      .catch(() => {
        setStatus('sign-out complete.');
        setTimeout(() => { window.location.href = 'https://shell.localhost:5201'; }, 1200);
      });
  }, []);
